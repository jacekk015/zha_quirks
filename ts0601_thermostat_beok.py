...
AVATTO_TARGET_TEMP_ATTR = 0x0210  # target room temp (decidegree)
BEOK_TEMPERATURE_ATTR = 0x0218  # current room temp (decidegree)
AVATTO_MODE_ATTR = 0x0402  # [0] manual [1] schedule
AVATTO_SYSTEM_MODE_ATTR = 0x0101  # device [0] off [1] on
BEOK_HEAT_STATE_ATTR = 0x0424  # [0] heating icon on [1] heating icon off
BEOK_CHILD_LOCK_ATTR = 0x0128  # [0] unlocked [1] locked
BEOK_TEMP_CALIBRATION_ATTR = 0x021B  # temperature calibration (degree)
...
class BeokManufCluster(TuyaManufClusterAttributes):
    """Manufacturer Specific Cluster of thermostatic valves."""

    def __init__(self, *args, **kwargs):
        """Init."""
        super().__init__(*args, **kwargs)
        global BeokManufClusterSelf
        BeokManufClusterSelf[self.endpoint.device.ieee] = self
    ...
    attributes.update(
        {
            BEOK_TEMPERATURE_ATTR: ("temperature", t.uint32_t, True),
            ...
            BEOK_HEAT_STATE_ATTR: ("heat_state", t.uint8_t, True),
            BEOK_CHILD_LOCK_ATTR: ("child_lock", t.uint8_t, True),
            BEOK_TEMP_CALIBRATION_ATTR: ("temperature_calibration", t.int32s, True),
        }
    )

    DIRECT_MAPPED_ATTRS = {
        BEOK_TEMPERATURE_ATTR: ("local_temperature", lambda value: value * 10),
        AVATTO_TARGET_TEMP_ATTR: (
            "occupied_heating_setpoint",
            lambda value: value * 100,
        ),
        BEOK_TEMP_CALIBRATION_ATTR: ("local_temperature_calibration", None),
    }
    ...
    def _update_attribute(self, attrid, value):
        ...
        if attrid == BEOK_CHILD_LOCK_ATTR:
        ...
        elif attrid == BEOK_TEMP_CALIBRATION_ATTR:
            self.endpoint.device.BeokTempCalibration_bus.listener_event(
                "set_value", value
            )
...
class BeokThermostat(TuyaThermostatCluster):
...
class BeokUserInterface(TuyaUserInterfaceCluster):
    """HVAC User interface cluster for tuya electric heating thermostats."""

    _CHILD_LOCK_ATTR = BEOK_CHILD_LOCK_ATTR


class BeokChildLock(CustomTuyaOnOff):
    ...
    def map_attribute(self, attribute, value):
        """Map standardized attribute value to dict of manufacturer values."""
        if attribute == "on_off":
            return {BEOK_CHILD_LOCK_ATTR: value}


class BeokTempCalibration(LocalDataCluster, AnalogOutput):
    """Analog output for Temp Calibration."""

    def __init__(self, *args, **kwargs):
        """Init."""
        super().__init__(*args, **kwargs)
        self.endpoint.device.BeokTempCalibration_bus.add_listener(self)
        self._update_attribute(
            self.attributes_by_name["description"].id, "Temperature Calibration"
        )
        self._update_attribute(self.attributes_by_name["max_present_value"].id, 20)
    ...
    async def write_attributes(self, attributes, manufacturer=None):
        ...
        for attrid, value in attributes.items():
            ...
            await BeokManufClusterSelf[
                ...
                {BEOK_TEMP_CALIBRATION_ATTR: value},
                manufacturer=None,
            )
        return ([foundation.WriteAttributesStatusRecord(foundation.Status.SUCCESS)],)


class Beok(TuyaThermostat):
    """Beok Thermostatic radiator valve."""

    def __init__(self, *args, **kwargs):
        """Init device."""
        self.thermostat_onoff_bus = Bus()
        self.BeokTempCalibration_bus = Bus()
        super().__init__(*args, **kwargs)
    ...
    replacement = {
        ENDPOINTS: {
            ...
            2: {
                ...
                INPUT_CLUSTERS: [BeokChildLock],
                OUTPUT_CLUSTERS: [],
            },
            3: {
                PROFILE_ID: zha.PROFILE_ID,
                DEVICE_TYPE: zha.DeviceType.CONSUMPTION_AWARENESS_DEVICE,
                INPUT_CLUSTERS: [BeokTempCalibration],
                OUTPUT_CLUSTERS: [],
            },
            242: {
                PROFILE_ID: 41440,
